...
public class Board {
    ...
    //board constructor
    Board(int numPlayers, int numScenes, int numDays) {
        this.numPlayers = numPlayers;
        this.numScenes = numScenes;
        this.numDays = numDays;

        //non-set rooms: these rooms won't have scenes nor shot counters in them
        this.casting = new Room("Casting", 0, temp);
        this.trailer = new Room("Trailer", 0, temp);

        //set rooms: these rooms do have scenes and shot counters in them.
        this.mainSt = new Room("Main Street", 3, temp);
        this.saloon = new Room("Saloon", 2, temp);
        this.bank = new Room("Bank", 1, temp);
        this.hotel = new Room("Hotel", 3, temp);
        this.church = new Room("Church", 2, temp);
        this.hideout = new Room("Secret Hideout", 3, temp);
        this.ranch = new Room("Ranch", 2, temp);
        this.trainStation = new Room("Train Station", 3, temp);
        this.jail = new Room("Jail", 1, temp);
        this.store = new Room("General Store", 1, temp);
        ...
        for (int i=0; i<cardList.getLength();i++) {
            ...
            for (int j=0; j< cardChildren.getLength(); j++){
              ...
              if("scene".equals(tagName.getNodeName())){
                 ...
                 Role role = new Role(partName, sLine, roleRank, false, 1);
            ...
            for (int j=1; j< setChildNodes.getLength(); j+=2) {
                ...
                if("parts".equals(parts.getNodeName())) {
                    for(int k=1; k<partList.getLength(); k+=2) {
                        ...
                        Node line = lineList.item(3);
                        String sLine = line.getTextContent();
                        Role eRole = new Role(name, sLine, level, false, 2);
    ...
    public void playGame(Player[] pList) {
        ...
        while(this.numDays > 0) { //while the game is still going
            for(int i = 0; i < pList.length; i++) { //taking turns
                pList[i].displayStats();
                System.out.println("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
                System.out.println(pList[i].getName() + "'s turn: ");
            ...
            if(numScenes == 1) { //if there is only one scene left, reset everything
                System.out.println("The day has ended, now returning all players back to Trailer...");
                this.numDays--;
                resetBoard();
                goBackHome(pList);
            }
